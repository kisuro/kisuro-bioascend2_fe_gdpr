...
export function SupplementReviewsModal({ supplement, isOpen, onClose }: SupplementReviewsModalProps) {
  ...
  const averageRating =
    supplement.ratings.length > 0
      ? supplement.ratings.reduce((sum, rating) => sum + rating.score, 0) / supplement.ratings.length
      : 0
  ...
  const ratingDistribution = [5, 4, 3, 2, 1].map((rating) => {
    const count = supplement.ratings.filter((r) => r.score === rating).length
    const percentage = supplement.ratings.length ? (count / supplement.ratings.length) * 100 : 0
